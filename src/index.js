...
app.get('/login', async(request, response)=> {
    ...
    if(!email){
        response.status(404).send(JSON.stringify({Mensagem: 'Insira um e-mail válido.'}))
        return;
    }
    if(!senha){
    ...
    if(!user){
        response.status(404).send(JSON.stringify({Mensagem: 'Email não encontrado no sistema, verifique ou crie uma conta.'}))
        return;
    }
    ...
    if(!senhas){
        response.status(404).send(JSON.stringify({Mensagem: 'Dados inválidos, digite novamente.'}))
        return;
    }

    response.status(200).send(JSON.stringify({Mensagem: `Seja bem vindo ${user.nome}! Pessoa usuária logada com sucesso!`}));
});
...
app.post('/message', (request, response)=>{
    ...
    if(!verificador){
        response.status(400).send(JSON.stringify({Mensagem: 'Email não encontrado, verifique ou crie uma conta.'}));
        return;
    }

    if(!title){
        response.status(400).send(JSON.stringify({Mensagem: 'Digite um título válido.'}))
        return;
    }
    if(!description){
        response.status(400).send(JSON.stringify({Mensagem: 'Digite uma descrição válida.'}))
        return;
    }
    ...
    response.status(201).send(JSON.stringify({Mensagem: `Mensagem criada com sucesso! Título: ${title}`}));
});
...
app.get('/message/:email', (request, response)=>{
    ...
    if(!verificar){
        response.status(400).send(JSON.stringify({Mensagem: 'Não existe mensagem cadastrada.'}));
        return;
    }
...
app.put('/message/:id', (request, response)=>{
    ...
    if(!id){
        response.status(400).send(JSON.stringify({Mensagem: 'Por favor, informe um ID válido da mensagem.'}));
        return;
    }
    ...
    if(!title){
        response.status(400).send(JSON.stringify({Mensagem: 'Informe um título válido.'}));
        return;
    }
    if(!description){
        response.status(400).send(JSON.stringify({Mensagem: 'Informe uma descrição válida.'}));
        return;
    }    
